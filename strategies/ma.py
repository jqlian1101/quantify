...
for index in range(len(ma_list)):
    ...
    graph_KAV.plot(np.arange(0, data_len),
                   df['ma%d' % cur_ma],
                   ma_line_color[index],
                   label='MA%d' % cur_ma,
                   lw=1.0)
...
# -------------------------------------------
# ------------------ 成交量 ------------------
graph_VOL.bar(np.arange(0, data_len),
              df.volume,
              color=[
                  'g' if df.open[x] > df.close[x] else 'r'
...
# ------------------------------------------
# ------------------ MACD ------------------
macd_dif, macd_dea, macd_bar = talib.MACD(df['close'].values,
                                          fastperiod=12,
                                          slowperiod=26,
                                          signalperiod=9)
# dif dea
...
graph_KDJ.set_xticklabels([
    df.index.strftime('%Y-%m-%d')[index] for index in graph_KDJ.get_xticks()
])  # 标签设置为日期
