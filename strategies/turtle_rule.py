...
history_data = query_history.QueryHistory('sh.600000', ('2019-1-1', '2020-1-1'),cols=["date","open","high","low","close","volume"])
...
for kl_index, today in df.iterrows():
    if today.signal == 1 and skip_days == 0:    # 买入
        ...
        plt.annotate('买入',xy=(kl_index,df.close.asof(kl_index)),xytext=(kl_index, df.close.asof(kl_index)+2),arrowprops=dict(facecolor='r',shrink=0.1),horizontalalignment='left',verticalalignment='top')
        ...
        if df.close[end] < df.close[start]:  # 赔钱显示绿色
            plt.fill_between(df.index[start:end], 0, df.close[start:end], color='green', alpha=0.38)
        else:  # 赚钱显示红色
            plt.fill_between(df.index[start:end], 0, df.close[start:end], color='red', alpha=0.38)


        plt.annotate('卖出',xy=(kl_index,df.close.asof(kl_index)),xytext=(kl_index+datetime.timedelta(days=5), df.close.asof(kl_index)+2),arrowprops=dict(facecolor='g',shrink=0.1),horizontalalignment='left',verticalalignment='top')
